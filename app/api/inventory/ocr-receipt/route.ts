...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!image) {
      ServerLogger.error(LogCategory.API, '画像ファイルが提供されていません');
    ...
    ServerLogger.info(LogCategory.API, '画像ファイル取得完了', { 
      fileName: image.name,
      fileSize: image.size,
      fileType: image.type
    });
    ...
    ServerLogger.info(LogCategory.API, 'Morizo AIにOCRリクエスト送信開始');
    ...
    // 3分のタイムアウトを設定（OCR解析は時間がかかる可能性があるため）
