              ...
              <div key={index}>
                ...
                {message.type === 'streaming' && message.sseSessionId && (
                  <StreamingProgress
                          ...
                          const updatedMessages: ChatMessage[] = prev.map((msg, idx) => 
                            idx === index
                              ? { 
                                  ...
                                  result: typedResult,
                                  requiresSelection: typedResult.menu_data?.requires_selection, // スネークケースをキャメルケースに変換
                                  candidates: typedResult.menu_data?.candidates,
                                  taskId: typedResult.menu_data?.task_id, // スネークケースをキャメルケースに変換
                                  sseSessionId: message.sseSessionId
                                }
                              : msg
                          );
                    ...
                    onError={(error) => {
                      // エラー時はエラーメッセージに置き換え
                      setChatMessages(prev => prev.map((msg, idx): ChatMessage => 
                        idx === index
